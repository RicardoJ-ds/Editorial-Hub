...
GRAIN: dict[str, dict[str, str]] = {
    ...
    "editorial_raw_clients": {"grain": "one row per client", "note": "client master + SF identity + pods"},
    "editorial_raw_deliverables": {"grain": "client × month", "note": "delivered / invoiced / sow_target per month"},
    "editorial_raw_production": {"grain": "client × month", "note": "actual vs projected article production"},
    "editorial_raw_goals": {"grain": "client × month × week × content_type", "note": "goals-vs-delivery weekly rows"},
    "editorial_raw_cumulative": {"grain": "one row per client", "note": "lifetime pipeline counters (topics/CBs/articles/published)"},
    "editorial_raw_capacity": {"grain": "pod × month × version", "note": "capacity projection inputs"},
    "editorial_raw_capacity_members": {"grain": "member × month", "note": "per-member capacity inputs"},
    "editorial_raw_client_pod_history": {"grain": "client × month", "note": "historical editorial-pod assignment per client"},
    "editorial_raw_articles": {"grain": "article × editor", "note": "article log; collaborations explode per editor"},
    "editorial_raw_article_revisions": {"grain": "one row per revision", "note": "revision events"},
    "editorial_raw_calendar": {"grain": "year × month × week", "note": "editorial calendar weeks"},
    "editorial_raw_kpi_scores": {"grain": "member × month × kpi_type", "note": "KPI scores vs target"},
    "editorial_raw_ai_monitoring": {"grain": "one row per monitored article", "note": "AI compliance monitoring records"},
    "editorial_raw_surfer_usage": {"grain": "Surfer API usage rows", "note": "Surfer SEO API usage"},
    "editorial_raw_team_members": {"grain": "one row per team member", "note": "roster (name, role, pod, capacity, email)"},
    "editorial_raw_delivery_templates": {"grain": "delivery template rows", "note": "pacing templates (fetched, currently unrendered)"},
    "editorial_raw_pod_history": {"grain": "year × month × kind × pod × client × role × person", "note": "raw per-month staffing history (editorial + growth)"},
    "editorial_raw_model_assumptions": {"grain": "model assumption rows", "note": "always-on capacity model assumptions"},
    "editorial_raw_name_mappings": {"grain": "kind × raw_name", "note": "editor/writer/client name → canonical dictionary"},
    # ── INT (business math computed here — the intermediate values) ──────────
    "editorial_int_client_months": {"grain": "client × month", "note": "the variance brain: delivered/invoiced/sow + Overview & D1 period assignment"},
    "editorial_int_client_q_snapshot": {"grain": "one row per client", "note": "lifetime + current-Q + last-full-Q variance on both Overview & D1 paths"},
    "editorial_int_goals_month_ct": {"grain": "client × month × content_type", "note": "weighted goals after max-of-week + contentTypeRatio"},
    "editorial_int_capacity_pod_months": {"grain": "pod × month × version", "note": "pod capacity mart (total / projected-used / actual-used)"},
    "editorial_int_member_months": {"grain": "member × month", "note": "member utilization mart"},
    "editorial_int_client_pod_months": {"grain": "client × pod × month", "note": "client contribution mart"},
    "editorial_int_articles_creation": {"grain": "editor × client × creation-month", "note": "monthly articles mart (editor credit)"},
    "editorial_int_articles_revisions": {"grain": "revision × month", "note": "monthly revisions mart"},
    "editorial_int_pod_assignments": {"grain": "year × month × kind × pod × client × role × person", "note": "resolved per-month staffing (editorial + growth) — the backfill surface"},
    # ── VIEWS (the public read contract — consumers read these) ──────────────
    "v_editorial_dim_client": {"grain": "one row per client", "note": "client master + SF identity (public)"},
    "v_editorial_dim_member": {"grain": "one row per member", "note": "roster dim"},
    "v_editorial_roster": {"grain": "one row per person × role", "note": "single-source editorial roster (Rippling editors + Slack writers + legacy); carries canonical work_email — DISPLAY this, key on canonical_name/slack_id"},
    "v_editorial_dim_calendar": {"grain": "year × month × week", "note": "calendar dim"},
    "v_editorial_fct_client_q_snapshot": {"grain": "one row per client", "note": "delivery/variance snapshot (Overview + D1)"},
    "v_editorial_fct_client_months": {"grain": "client × month", "note": "per-month delivery/variance detail"},
    "v_editorial_fct_pod_snapshot": {"grain": "pod_axis × pod", "note": "Overview Pod Snapshot rollup, both pod axes"},
    "v_editorial_fct_goals_monthly": {"grain": "client × month × content_type", "note": "weighted goals (raw + weighted measures)"},
    "v_editorial_fct_goals_client_totals": {"grain": "one row per client", "note": "goal totals gated on weighted goal > 0"},
    "v_editorial_fct_production_monthly": {"grain": "client × month", "note": "production actual vs projected + pods"},
    "v_editorial_fct_pipeline": {"grain": "one row per client", "note": "pipeline counters (topics/CBs/articles/published)"},
    "v_editorial_fct_milestone_transitions": {"grain": "client × transition", "note": "8 milestone transitions, calendar-day diffs"},
    "v_editorial_fct_kpi_scores": {"grain": "member × month × kpi_type", "note": "KPI scores + member/role/pod"},
    "v_editorial_fct_pod_assignments": {"grain": "year × month × pod × client × role × person", "note": "resolved editorial-only per-month staffing"},
    "v_editorial_fct_capacity_pods": {"grain": "pod × month × version", "note": "pod capacity (public)"},
    "v_editorial_fct_member_utilization": {"grain": "member × month", "note": "member utilization (public)"},
    "v_editorial_fct_client_contributions": {"grain": "client × pod × month", "note": "client contribution (public)"},
    "v_editorial_fct_articles_monthly": {"grain": "editor × client × creation-month", "note": "monthly articles (editorial-month basis)"},
...
def introspect(with_counts: bool = True) -> list[dict]:
    ...
    if with_counts:
        for name in objects:
            ...
            select = "COUNT(*) AS n" + (", MAX(synced_at) AS fresh" if has_synced else "")
            try:
...
def write_json(catalog: list[dict], generated_at: str, live: bool) -> None:
    payload = {
        ...
        "source": "live BigQuery INFORMATION_SCHEMA" if live else "SEED (BQ unreachable — from build.py/views.py)",
        "counts": {
...
def _key_columns(cols: list[dict], limit: int = 8) -> str:
    ...
    priority = [
        n
        for n in names
        if any(k in n for k in ("id", "name", "pod", "year", "month", "date", "kind", "role", "status", "person"))
        and n != "synced_at"
    ]
    rest = [n for n in names if n not in priority and n != "synced_at"]
    picked = (priority + rest)[:limit]
    suffix = " …" if len(names) - (1 if "synced_at" in names else 0) > len(picked) else ""
...
def write_md(catalog: list[dict], generated_at: str, live: bool) -> None:
    ...
    lines: list[str] = []
    lines.append("# BQ schema catalog — `graphite-data.graphite_bi_sandbox` (editorial warehouse)")
    lines.append("")
    lines.append(
    ...
    src = "live BigQuery `INFORMATION_SCHEMA`" if live else "⚠️ **SEED** — BQ was unreachable; columns & counts omitted, grains from build source"
    lines.append(f"- **Source:** {src}")
    lines.append(f"- **Inventory:** {tables} tables · {views} views · {columns} columns")
    lines.append("")
    lines.append(
    ...
    for fam in sorted(by_family, key=lambda f: FAMILY_ORDER.get(f, 9)):
        ...
        for e in entries:
            ...
            lines.append(f"| `{e['name']}` | {e['grain']} | {rows} | {fresh} | {detail} |")
        lines.append("")
...
def sync_to_planning_hub() -> list[str]:
    ...
    if not PLANNING_HUB_DOCS.exists():
        print(f"  ! planning-hub docs dir not found: {PLANNING_HUB_DOCS} — skipping sync")
        return []
    copied = []
...
def main() -> None:
    ap = argparse.ArgumentParser(description="Generate the BQ schema catalog for the planning-hub.")
    ap.add_argument("--no-sync", action="store_true", help="don't copy artifacts into the planning-hub docs/ folder")
    ap.add_argument("--no-counts", action="store_true", help="skip per-table row counts / freshness")
    args = ap.parse_args()
    ...
    try:
        ...
        print("    Emitting SEED catalog from build.py/views.py grain map (no live columns/counts).")
