...
**Current version: `0.3.11`** — see `CHANGELOG.md` for the full history and the
...
| `/overview` | **Overview** (exec snapshot) | Two primary sections: **Pod Snapshot** (per-pod Delivery Progress + Milestone Journey, cumulative-through-end-of-Q variance with tier + pace-coloured bar, section-local Goals period selector, click-anchored `ClientDetailPopover` drill-downs) and **Production History** (`ProductionTrendChart` with All / Per-pod view toggle that respects the global Pod Axis and uses `POD_HEX_COLORS`). Everything else (Time-to Metrics, Delivery Overview, Cumulative Pipeline, Client Delivery) is parked inside a single `LegacySectionsContainer` behind a Show toggle — superseded by Pod Snapshot, kept for reference until removal. Each visible section deep-links into D1. |
