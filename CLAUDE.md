...
**Current version: `0.3.49`** — see `CHANGELOG.md` for the full history and the
...
| Delivery Schedules | `delivery_templates` | ✅ live sync (**Re-sync Past Months** — per-SOW-size article distribution templates 240/220/180/120/125, change rarely); drives Pacing badge |
...
`ARTICLE_COUNT_ID` = `15WxjOrJtX98YvnWl71Oz0HtVhPtvefB37PbcmAc8BBc` — the **REAL original** "[Internal] Monthly Article Count/Revenue Sheet" (config.py default; **Railway also sets this var — keep both in sync**). **As of 0.3.41 the Hub ingests this original directly** (no longer the proposal copy `1TIfMsXr…`): `etl/apply_original.py` normalized it in **REPLACE mode** — EDITOR/WRITER are canonical at the source, and the single `REVISED` column was split into `1ST/2ND/3RD REVISION`. The importer reads those via the `REVISED_1/2/3` aliases, plus `REVISED` / `REVISED DATE` for legacy/variant tabs (all header-driven + order-tolerant). The editor/writer dropdowns validate against an IMPORTRANGE'd local `📋 Roster` (non-strict — "/" collaborations + legacy names warn, don't block); names with no canonical entry stay raw + surface in DQ → Article mappings. **The master `Roster` tab is generated from the BigQuery view `graphite_bi_sandbox.v_editorial_roster`** (single source of truth — `etl/warehouse/v_editorial_roster.sql`): Rippling editors (`v_headcount`, `title LIKE '%editor%'`; Sr./Lead/Director/VP → `sr_editor` for the 2nd-review list) + Slack writers (`slack_raw_users`, `ext.writing` email) + a legacy bucket (confirmed `editorial_name_map` canonicals not in either), names canonicalised via `editorial_name_map`, **minus a DaniQ-editable Exclusions tab** (published to BQ `editorial_roster_exclusions` on the `@name-mappings` SYNC step, role-aware — that's how a "not an editor" with an `Editorial Lead` title is removed permanently without losing them as a writer). The Apps Script `etl/apps_script/roster_refresh.gs` (daily trigger, graphite-tools project) materialises the view → master Roster tab (active-first) → IMPORTRANGE → MAC dropdowns; replaces the old hand-regenerated roster. The proposal copies (`1TIfMsXr…` MAC + `1eiFh7…` Meta) are now demo-only. **Deprecated/trashed — never point at them:** `1X_M82VzstJCulkl6l62jaubn2yI0ODBTz33iZ4XqZWU` (was Railway's stale value, now 404), `1eRmZFnrhhPdHwkvLm9Ho1aNMq4WoyeggVhpShrW0kL0` ("Copy Normalized of…", trashed), older `1FWykZmeG2jznUYn-ng6glN4wjvc1Swb6hHmSHzGZ7dU`. Copies don't inherit SA sharing — if a sheet 403s, enumerate via the Drive API + check `trashed`. **One tab per client** (~100 tabs, hidden tabs included — paused/ended clients), ~15K delivered-article rows. Ongoing cell-level date/revision hygiene on the source MAC runs via `etl/date_audit.py` (reads UNFORMATTED so a real date is a serial vs typed text; recovers text submission dates, fixes wrong-year serials validated against chronological order, retires legacy duplicate revision columns, relocates non-date junk — every original preserved in a `SUBMISSION NOTES` / `REVISION NOTES` column).
...
**Meta is a separate source folded into the SAME rebuild** (since 0.3.30): Meta's article log lives in its own **"Meta Editorial Tracker"** sheet (`META_TRACKER_ID`, single `TRACKER` tab, rows grouped by a `VERTICAL` column — **AI / Reality Labs / for Business**), NOT in the per-client tabs. `_parse_meta_tracker()` (called inside `import_monthly_article_count`, before the Notion match + the single delete-insert) maps the 3 verticals to the existing Hub clients **Meta AI / Meta RL / Meta BMG** (all editorial Pod 5; current-pod fallback so out-of-coverage months still surface) and builds rows with the identical parsing (only Meta-specific bit: the separate `REVISION 1/2/3` columns are joined into the comma-list the shared parser expects). Closes the long-standing "Meta has no tab" gap (~204 articles). The Meta sheet also carries the same normalization columns via `python -m etl.sheet_standardize --meta` (WRITER/EDITOR (STANDARD) + 1ST/2ND/3RD REVISION (STANDARD) + **2ND REVIEW (STANDARD)** — a strict Sr-editor dropdown, active first, for the relatively-new 2nd-review field; inline `ONE_OF_LIST` roster dropdowns since cross-spreadsheet ranges aren't allowed, + strict date validation). **Both proposal copies** (Monthly Article Count `ARTICLE_COUNT_ID` + Meta `META_TRACKER_ID`) draw their editor/writer rosters + WRITER (STANDARD) fill from the **live BigQuery `editorial_name_map`** (DaniQ-editable sheet → BQ via the `@name-mappings` SYNC step), so her corrections (incl. the "Auditioning Writer" bucket) flow into the proposal sheets, the importer, the warehouse writer canon, and the Notion-KPI editor matching — every place a name is mapped reads the one BQ source. Capacity matching inherits canonical editor names transitively via `article_records`.
