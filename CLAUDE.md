...
ID env-driven via `ARTICLE_COUNT_ID` = `1X_M82VzstJCulkl6l62jaubn2yI0ODBTz33iZ4XqZWU` — the canonical **"Copy of [Internal] Monthly Article Count/Revenue Sheet"** DaniQ maintains. Shared with the BI service account on **2026-06-23** after a sharing gap had silently failed prod MAC imports (0 rows); confirm SA access before any sheet op. **Deprecated/trashed copies — do NOT point `ARTICLE_COUNT_ID` at them:** `1eRmZFnrhhPdHwkvLm9Ho1aNMq4WoyeggVhpShrW0kL0` ("Copy **Normalized** of …", now in trash) and the older `1FWykZmeG2jznUYn-ng6glN4wjvc1Swb6hHmSHzGZ7dU`. (Several near-identical copies exist; copies don't inherit SA sharing — enumerate via the Drive API + check `trashed` if a sheet 403s.) **One tab per client** (~100 tabs, hidden tabs included — paused/ended clients), ~15K delivered-article rows.
