...
**Current version: `0.3.39`** — see `CHANGELOG.md` for the full history and the
...
`ARTICLE_COUNT_ID` is a **config.py default** (not a Railway/local env var) = `1TIfMsXr7Vpoip4NcEWMlPaTrj3F4VTXZKk7cVlxKYM0` — the current canonical **"Copy of [Internal] Monthly Article Count/Revenue Sheet"** DaniQ maintains (swapped 2026-06-24; SA-accessible). DaniQ periodically trashes the old copy and makes a fresh one, so **the ID changes — update this default + confirm SA access before any sheet op.** **Trashed/deprecated copies — never point at them:** `1X_M82VzstJCulkl6l62jaubn2yI0ODBTz33iZ4XqZWU` (prev, trashed 2026-06-24), `1eRmZFnrhhPdHwkvLm9Ho1aNMq4WoyeggVhpShrW0kL0` ("Copy Normalized of…", trashed), older `1FWykZmeG2jznUYn-ng6glN4wjvc1Swb6hHmSHzGZ7dU`. Copies don't inherit SA sharing — if a sheet 403s, enumerate via the Drive API + check `trashed`. **One tab per client** (~100 tabs, hidden tabs included — paused/ended clients), ~15K delivered-article rows.
